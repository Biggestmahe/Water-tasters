...
stationsover = stations_level_over_threshold(stations, 0.9)
...
for station in stationsover:
    ...
    if dates == [] or levels == []:
        ...
        derivative = np.polyder(poly, 2)
        value = derivative(plt.dates.date2num(dates[-1]) - d0)
        if station[0].town == None :
            pass
        elif station[1] > 1.5 and value > 0:
            severe.add(station[0].town)
        elif station[1] > 1.3 and value > 0:
            high.add(station[0].town)
        elif station[1] > 1.1 or value > 0:
